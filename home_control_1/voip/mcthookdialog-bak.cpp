...
void MctHookDialog::ResetState(QString text, QString strIp, int vchannel)
{
...
/*void MctHookDialog::on_tableViewVideoChannel_clicked(QModelIndex index)
{
	...
	usleep(400000);
	lpc_cmd_mon(lphone, (m_strIp + ":" + QString::number(vchannel)).toLatin1().data());
...
void MctHookDialog::on_tableWidgetVideoChannel_itemSelectionChanged()
{
	...
	vchannel = ui->tableWidgetVideoChannel->item(index.row(), index.column())->text().toInt();
