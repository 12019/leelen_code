...
int main()
{
    ...
    /*FILE *fp = fopen("/mnt/disk/tv_capture.yuv", "wb");
	if (!fp)
	{
	...
	char *img;
    img = (char *)cedar_sys_phymalloc_map(720 * 576 * 3 / 2, 1024);
    ...
    while (1)
    {
        ...
        {
            img[720 * 576 + j * 2] = ptr[j * 2 + 1];
            img[720 * 576 + j * 2 + 1] = ptr[j * 2];
        }
        */
        /*fwrite(mapMem.mem[buf.index], 1, 720 * 576, fp);
        fwrite(tmp_buf, 1, 720 * 576 / 2, fp);
        fflush(fp);        
        */
		...
		pic.size_y = 720 * 576;
		pic.size_u = 720 * 576 / 4;
		pic.size_v = 720 * 576 / 4;

		pic.y = (u8 *)phy_img;
		pic.u = (u8 *)phy_img + 720 * 576;
		pic.v = (u8 *)phy_img + 720 * 576 * 5 / 4;
        
        render_render((void *)&pic, i, 0, 0, 800,480);
